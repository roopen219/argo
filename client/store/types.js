// TABS
export const ADD_TAB = 'tabs/ADD_TAB'
export const REMOVE_TAB = 'tabs/REMOVE_TAB'
export const SWITCH_TAB = 'tabs/SWITCH_TAB'
export const ADD_TAB_GROUP = 'tabs/ADD_TAB_GROUP'
export const REMOVE_TAB_GROUP = 'tabs/REMOVE_TAB_GROUP'
export const UPDATE_TAB_CONTENT = 'tabs/UPDATE_TAB_CONTENT'
export const REPLACE_TAB_CONTENT = 'tabs/REPLACE_TAB_CONTENT'

// PROTOTYPE
export const FETCH_PROTOTYPES = 'prototype/FETCH_PROTOTYPES'
export const CREATE_PROTOTYPE = 'prototype/CREATE_PROTOTYPE'
export const SAVE_PROTOTYPE = 'prototype/SAVE_PROTOTYPE'
export const DELETE_PROTOTYPE = 'prototype/DELETE_PROTOTYPE'

export const OPEN_PROTOTYPE = 'prototype/OPEN_PROTOTYPE'
export const CLOSE_PROTOTYPE = 'prototype/CLOSE_PROTOTYPE'

export const ADD_PROTOTYPE = 'prototype/ADD_PROTOTYPE'
export const REMOVE_PROTOTYPE = 'prototype/REMOVE_PROTOTYPE'

export const SELECT_ELEMENT = 'prototype/SELECT_ELEMENT'
export const CREATE_ELEMENT = 'prototype/CREATE_ELEMENT'
export const APPEND_ELEMENT = 'prototype/APPEND_ELEMENT'

// APP


// EDITOR

export const EDITOR_ADD_PROTOTYPE = 'editor/ADD_PROTOTYPE'
export const EDITOR_CLOSE_PROTOTYPE = 'editor/CLOSE_PROTOTYPE'

export const ADD_ELEMENT_TO_SELECTION = 'editor/ADD_ELEMENT_TO_SELECTION'

export const REMOVE_ELEMENT = 'editor/REMOVE_ELEMENT'

